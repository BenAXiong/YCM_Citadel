...
def ensure_table(db_path="export/games_master.db"):
...
def is_harvested(dialect_id, db_path="export/games_master.db"):
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def save_dialect_to_db(d_id, d_name, words, db_path="export/games_master.db"):
    if not words: return
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    ...
    rows = []
    for w in words:
        rows.append((d_id, d_name, w.get("word_ab","").strip(), w.get("word_ch","").strip(), w.get("source",""), w.get("num",0)))
    
    cur.executemany("INSERT INTO ilrdf_vocabulary (dialect_id, dialect_name, word_ab, word_ch, source, num) VALUES (?,?,?,?,?,?)", rows)
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    for d_id, d_name in sorted(dialects):
        ...
        save_dialect_to_db(d_id, d_name, words)
        total += len(words)
        time.sleep(0.5)
