...
/**
 * MOE HIERARCHY BUILDER (The Architect)
 * 
 * GOAL: Generate a unified, pre-calculated morphological manifest for the Kilang View.
 * STRATEGY: Prioritize linguistic dictionary data (stems) over heuristics.
 * METHOD: 
 *   1. Priority 1: Official DB Stem (Explicitly tagged in MoEDICT).
 *   2. Priority 2: Global Substring Match (Longest word found within the current word).
 *   3. Resolution: Trace every word back to an Ultimate Root (Ancestor with no parent).
 * OUTCOMES:
 *   - moe_morph_manifest.json: A mapping of every word to its parent and ultimate root.
 *   - moe_morph_stats.json: Aggregated statistics for the Explorer/Distribution headers.
 */
...
sortedWords.forEach(word => {
  ...
  manifest[word] = {
    w: wordToEntry.get(word),
    p: parentMap.has(word) ? wordToEntry.get(parentMap.get(word)) : null,
    r: wordToEntry.get(ultimateRoot),
    d: depth
  };
...
const manifestPath = 'c:/Users/Ben/Documents/LL/6_ycm/YCM_Citadel/portal/public/data/moe_morph_manifest.json';
const statsPath = 'c:/Users/Ben/Documents/LL/6_ycm/YCM_Citadel/portal/public/data/moe_morph_stats.json';

fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
fs.writeFileSync(statsPath, JSON.stringify(stats, null, 2));

// 5. Database Injection (The Single Source of Truth Sync)
console.log('- Syncing manifest to Database (moe_hierarchy table)...');

db.transaction(() => {
  db.prepare("DROP TABLE IF EXISTS moe_hierarchy").run();
  db.prepare(`
    CREATE TABLE moe_hierarchy (
  ...
  const insert = db.prepare("INSERT INTO moe_hierarchy (word_ab, parent_word, ultimate_root, depth) VALUES (?, ?, ?, ?)");
