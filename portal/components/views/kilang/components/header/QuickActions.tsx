          ...
          <div className={`absolute left-full ml-1 flex items-center gap-2 p-1.5 transition-all duration-300 origin-left z-[100] ${isPaletteHovered ? 'opacity-100 scale-100 translate-x-0' : 'opacity-0 scale-90 -translate-x-2 pointer-events-none'}`}>
            <div className="flex items-center px-2 py-1 gap-1">
              {THEME_PRESETS.map((t, idx) => (
                ...
                >
                  <div
                    className="absolute inset-1 blur-2xl opacity-80 group-hover/item:opacity-100 transition-all duration-500 rounded-full z-0"
                    style={{ backgroundColor: t.color }}
                  />
                  <div
                    className="w-[30px] h-[30px] relative z-10 flex items-center justify-center opacity-80 group-hover/item:opacity-100 transition-opacity"
                    style={{ filter: `drop-shadow(0 0 12px ${t.color}88)` }}
                  >
                    <img
                      src="/kilang/Kilang_5_nobg.png"
                      alt=""
                      className={`w-full h-full object-contain ${state.layoutConfig.theme === t.id ? 'brightness-150' : 'brightness-100'}`}
                    />
                  </div>
                </button>
              ))}

              {/* Popout Button - Radiant Ghost Style */}
              <button
                onClick={openThemePopout}
                className="w-10 h-10 flex items-center justify-center rounded-full text-white/50 hover:text-white transition-all active:scale-90"
